...
void makeList(const char* s, Node*& head, Node*& tail) {
    ...
    while (*s != '\0') {
        Node* newNode = new Node{.ch = *s, .next = nullptr};
...
int main() {
    ...
    makeList("Kyoungmin", nameHead, nameTail); // You can freely change the input parameter as your name.
